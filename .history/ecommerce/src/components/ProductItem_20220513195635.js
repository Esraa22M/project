...
import { Product, LinkContainer } from "../styles/ProdctItem.style";
      ...
      <Product>
        <LinkContainer to={`/details/${id}${this.props.categoryId}`}>
          <div className="div-1">
            ...
            <p>{name}</p>
            <h2 className="price">
              {prices[this.props.selectedCurrencyIndex].currency.symbol}{prices[this.props.selectedCurrencyIndex].amount}
            </h2>
          </div>
        </LinkContainer>
        {!inStock ? (
          <div className="div-2">OUT OF STOCK</div>
        ) : (
          <div
            className="card-icon"
            // onClick={() => {
            //   this.handleProductIdAndCategoryId(id);
